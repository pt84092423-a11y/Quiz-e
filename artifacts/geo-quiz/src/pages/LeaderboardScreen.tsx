...
import { getLeaderboard, clearLeaderboard, LeaderboardEntry } from '../lib/leaderboard';
      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto flex flex-col flex-1 p-4 md:p-8">
        ...
        {entries.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex-1 flex flex-col items-center justify-center text-center text-muted-foreground gap-4"
          >
            ...
            {entries.map((entry, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex items-center gap-4 p-4 rounded-2xl border backdrop-blur-xl ${style.bg} ${style.border}`}
                  data-testid={`row-leaderboard-${rank}`}
                >
                  {/* Rank number */}
                  <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center font-black text-lg ${style.bg} ${style.text} border ${style.border}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className={`text-xl font-black tabular-nums ${rank <= 3 ? style.text : 'text-foreground'}`}>
                        #{rank}
                      </span>
                      <span className="text-muted-foreground text-sm truncate">{entry.date}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                      <span className="text-foreground font-semibold">
                        {entry.correct}/{entry.total} câu đúng
                      </span>
                  ...
                  <div className="shrink-0 text-right">
                    ...
                    <div className="text-xs text-muted-foreground uppercase tracking-wider">điểm</div>
                  </div>
                </motion.div>
              );
            })}
